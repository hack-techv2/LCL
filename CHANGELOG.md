...
**Follow-up (same day):** once a preview exists the raw source is now **collapsed by default** with a **Show/Hide HTML** (or "source") toggle beside Hide preview — the reader sees the rendered result first, code on demand. The inline preview grew from a cramped 340px box to a **screen's-worth (~85vh)**, and the revealed source matches it (up to 85vh), so with both open they stack as two full-height panes rather than splitting one. Replaced the fullscreen idea with an **Open in browser** button (HTML only): it opens the page in a **real new browser tab** at full window size, but the tab hosts the *same* sandboxed iframe (`allow-scripts` + the shared CSP-injected doc `codeHtmlDoc`) inside a script-free shell — so the page gets the whole window yet still can't read LCL's key/storage or reach the proxy. Test **C64** guards collapsed-by-default, the (tall) source toggle, and that the new tab stays sandboxed on the shared CSP document.
...
**Rate-limit UX — no more hammering a drained token window.** With very long chats each turn re-sends the full history (~84k tokens seen in the 16 Jul log), so the shared 200k/min window drains and sends 429 until it resets. Two changes make that graceful: (1) **pre-send guard** — if a recent 429's window has not reset yet, sending warns "resets in ~Ns — wait or send anyway?" instead of firing into a window that will just reject it (`rlWindowUntil`, self-expiring, set from the 429 reset and cleared on a clean success); (2) **re-sending during a countdown no longer cancels + re-fires** it — it's blocked with a toast ("retrying automatically in ~Ns; press Stop to cancel and send this instead"), so the auto-retry the app already scheduled isn't thrown away into another guaranteed 429. Tests **C50** + the pre-send guard. Client-only.

Known/deferred: the first-attempt upstream inactivity timeout is 10s, which can spuriously trip on very large prompts (>10s to first token) → a 502 that auto-retries; raising the first-byte budget is proposed but not built this round.

Suites: client-logic 48/48 (+C49, +C50), demo-api 42/42, build 5/5.

## 15 Jul 2026 - Replies finish in the background when you switch chats + budget-exhaustion 429 no longer retries forever (alpha)

**Fix — switching chats mid-reply no longer errors into the wrong chat.** Starting a reply and then opening another chat used to abort the stream and then leak its wrap-up — a stray "(stopped)"/error bubble and a red "Replying" health pill — into the chat you switched *to* (the append targets whatever chat is on screen). Now a run is scoped to the chat it started in: `switchChat`/`newChat` no longer abort it, and `runStream` (plus the split-summary runner) only touch the live message view and health pill while their own chat is on screen. The reply keeps generating into its origin chat and is there — complete — when you switch back; nothing appears in the chat you moved to. Press **Stop** (button / Esc) to actually cancel. A 429/5xx that arrives while the chat is backgrounded retries silently instead of drawing a countdown in the wrong chat. The health pill resyncs on switch and when a background run ends, so it never sticks on "Replying" once nothing is generating. Verified live in `#demo` (mid-stream switch: origin chat completed to full length, the other chat stayed clean, pill cleared, zero console errors).

**Fix — "budget(s) exceeded" 429 was retried every ~63s for 10+ hours.** An overall **API-key budget exhaustion** comes back as a flat `{"error":"1 budget(s) exceeded"}` 429 with none of the per-minute rate-limit fields (no reset time / current limit / remaining). `postClassified` classified every 429 as `ratelimit` and fell into a fixed 60s wait-and-retry, so it spammed the exhausted key forever (15 Jul debug log: the identical 429 recurring for 10+ hours). Such a 429 is now classified **terminal** — surfaced as a plain error ("API key budget exhausted — … check your key's quota or switch to another key") with **no countdown and no auto-retry**. New demo marker `[[budgetexceeded]]`. Regression tests **C48** (client classification) and **T42** (demo server shape).

Suites: demo-api 41/41 (+T42 = 42 cases), client-logic 46/46 (+C48), build 5/5. Client-only except the demo marker in `server.txt` (a Node restart is only needed to exercise `[[budgetexceeded]]` live in `#demo`; the real budget-error handling is all client-side).
...
## 9 Jul 2026 - Fix: most recent chat lost after an alpha update restart (alpha)

Applying an alpha update (or switching channels) asked the server to restart Node **without first flushing pending state to disk**, so a chat save still moving through the write queue - typically the most recent chat/reply - was dropped when the process died. All three restart paths (`applyAlphaNow`, channel switch, simulate) now `await flushBeforeRestart()` first, which awaits `persist()` (the serialized `/api/data` write chain) so the latest `D` is guaranteed on disk before the restart request. Test **C47**. Build 5/5, client-logic 45/45.
...
## 9 Jul 2026 - Health pill shows + OCR label (not the progress count) (alpha)

Refinement to the earlier pill change: the connection health pill again shows **"+ OCR"** when the engine is on (and drops it when off), refreshing the moment you toggle it. What stays off the pill is the live **"n/N" progress** during an OCR run - that count lives only on the OCR chip, so the same number never appears in two places. Test **C39** updated. Build 5/5, client-logic 40/40.

## 9 Jul 2026 - Unified popup look across LCL (alpha)

The centered popups had drifted apart: corners ranged 14-20px (confirm dialogs 14, settings 16, connect modal 20), backdrops ranged 45-65% black, and blur ranged 0-8px (the confirm dialogs had no blur at all). They now share one spec - **16px corners, a 50% black backdrop with 6px blur, and the same 1px border + `0 20px 60px` shadow** - so the connect modal, search, confirm dialogs, and settings/skills panels all feel like the same product. Padding still varies by content. The small anchored info popovers (OCR, RAG) keep their own compact style. Test **C42** guards the shared spec.

## 9 Jul 2026 - Consistent popup buttons (alpha)

Confirmation popups (delete chat, embed batch, and the OCR row dialog) had a full-width orange primary button that was taller and wider than the outlined Cancel beside it, because the primary reused the connect modal's `btn-p` style (`width:100%`, `11px` padding, no border). All dialog primaries now use a self-contained `cd-ok` chip that matches the `cd-cancel` height and width exactly (same padding, 1px border, 13px text), and the action rows are vertically centred - so Cancel and the primary line up cleanly. Test **C41** guards it. Build 5/5, client-logic 39/39.

## 9 Jul 2026 - OCR dialog redesign + remembers your on/off choice (alpha)

Two OCR-UX changes. (1) The scanned-file dialog was redesigned for a cleaner, more consistent look: instead of three mismatched buttons crammed in a row (a full-width orange primary next to two identical grey outlines), the choices are now stacked full-width with a clear hierarchy - a file chip showing the scanned file, then **Run OCR, then attach/embed** (primary) with a one-line hint, **Attach/Embed without OCR** (secondary), and a quiet **Cancel** (ghost). (2) OCR now remembers whether you turned it on: the preference persists and the engine **auto-enables on the next page load**, so a refresh no longer silently drops back to Off. The engine bytes still can't be cached on this network (Edge Tracking Prevention), but the CDN assets are usually served from the browser HTTP cache, so the re-enable is quick after the first load; a blocked network shows the normal blocked state. Test **C40** guards both. Build 5/5, client-logic 38/38.

## 9 Jul 2026 - OCR clarity: client-side note, drop redundant status, honest health pill (alpha)

Three small OCR-UX fixes. (1) The OCR popover now states everything runs **locally in your browser** - nothing is uploaded to extract the text - **and you can keep using LCL while OCR runs**. (2) When the engine toggle is green (On), the redundant "Ready" status line is gone; the green "On" already says it (kept for idle/downloading/blocked). (3) The connection health pill no longer echoes OCR at all - neither the "+ OCR" label nor the "OCR n/N" progress (which previously showed the same count in two places at once). The OCR chip is now the single place OCR status and progress appear; the pill shows just "Chat" or "Chat + embed". Test **C39** guards all three. Build 5/5, client-logic 37/37.
