...
**Follow-up (same day):** once a preview exists the raw source is now **collapsed by default** with a **Show/Hide HTML** (or "source") toggle beside Hide preview — the reader sees the rendered result first, code on demand. The inline preview grew from a cramped 340px box to a **screen's-worth (~85vh)** so normal pages render full-size without an inner scrollbar. Added a **Fullscreen** button that fills the whole browser/monitor via the Fullscreen API, requested on the parent-owned `.code-wrap` (so the toolbar/Exit ride along) — the sandbox is **not** widened to `allow-fullscreen`, so the previewed page still can't self-fullscreen and stays fully isolated; Esc exits. Test **C64** guards collapsed-by-default, the source toggle, fullscreen-on-wrap, and that the sandbox is never widened.
