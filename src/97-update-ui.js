...
async function applyUpdate(){
  if (typeof demoOn === 'function' && demoOn()) {
    ...
    setTimeout(() => { if (out) out.innerHTML = '<div style="color:var(--ok);padding:6px 0">Updated to v0.67f. Restarting Node\u2026</div>' }, 1000)
    setTimeout(() => {
      const vb = document.getElementById('ver-badge'); if (vb) vb.textContent = 'v0.67f'
      lclUpdate = { checked:true, channel:'stable', current:'0.67f', latest:'0.67f', tag:'v0.67f', newer:false, error:null, ref:'alpha', inSync:true, changed:[], hash:'' }
      renderUpdateBadge(); renderUpdateSettings(); closeUpdateDialog()
      toast('Updated to v0.67f (demo) \u2014 Reset demo to replay', 'ok')
    }, 2100)
    return
  }
  const btn = document.getElementById('update-apply-btn')
...
async function setChannel(ch){
  if (typeof demoOn === 'function' && demoOn()) {
    ...
    if (ch === 'alpha') {
      ...
      lclUpdate = { checked:true, channel:'alpha', current:'0.67f', ref:'alpha', inSync:true, changed:[], hash:'a1b2c3d', installedAt:Date.now(), error:null, sameAsStable:true }
      if (typeof toast === 'function') toast('Enrolled in Experimental updates \u2014 already on the latest build','ok')
    } else {
      lclUpdate = { checked:true, channel:'stable', current:'0.67e', latest:'0.67f', tag:'v0.67f', newer:true, inSync:true, changed:[], hash:'', error:null }
      relockAlpha()
    }
...
async function applyAlphaNow(){
  if (typeof demoOn === 'function' && demoOn()) {
    ...
    setTimeout(() => {
      lclUpdate = { checked:true, channel:'alpha', current:'0.67e', ref:'alpha', inSync:true, changed:[], hash:'e4f5a6b', installedAt:Date.now(), error:null }
