...
const MODEL_TIERS = {
  cce: [
    { label: 'Claude (Anthropic)', ids: [
      'azure.claude-haiku-4-5','azure.claude-opus-4-5','azure.claude-opus-4-6','azure.claude-opus-4-7',
      'azure.claude-opus-4-8','azure.claude-opus-5','azure.claude-sonnet-4-5','azure.claude-sonnet-4-6',
      'azure.claude-sonnet-5','rsn.claude-haiku-4-5','rsn.claude-opus-4-5','rsn.claude-opus-4-6',
      'rsn.claude-opus-4-7','rsn.claude-opus-4-8','rsn.claude-opus-5','rsn.claude-sonnet-4-5',
      'rsn.claude-sonnet-4-6','rsn.claude-sonnet-5','bedrock.claude-3-5-sonnet','bedrock.claude-haiku-4-5',
      'bedrock.claude-opus-4-5','bedrock.claude-opus-4-6','bedrock.claude-opus-4-7','bedrock.claude-opus-4-8',
      'bedrock.claude-opus-5','bedrock.claude-sonnet-4-0','bedrock.claude-sonnet-4-5','bedrock.claude-sonnet-4-6',
      'bedrock.claude-sonnet-5','cce.claude-3-5-sonnet','rsn.claude-sonnet-4-0','cce.claude-opus-4-5',
      'cce.claude-opus-4-6','cce.claude-sonnet-4-5','cce.claude-sonnet-4-6','rsn.claude-opus-4-1',
      'rsn.vertex_ai.claude-opus-4-5','rsn.vertex_ai.claude-opus-4-6','rsn.vertex_ai.claude-sonnet-4-5','rsn.vertex_ai.claude-sonnet-4-6',
      'vertex_ai.claude-haiku-4-5','vertex_ai.claude-opus-4-1','vertex_ai.claude-opus-4-5','vertex_ai.claude-opus-4-6',
      'vertex_ai.claude-opus-4-7','vertex_ai.claude-opus-4-8','vertex_ai.claude-opus-5','vertex_ai.claude-sonnet-4-5',
      'vertex_ai.claude-sonnet-4-6','vertex_ai.claude-sonnet-5'
    ] },
    { label: 'OpenAI', ids: [
      'gpt-5.2-chat','gpt-4.1','gpt-4.1-mini','gpt-4.1-nano',
      'gpt-4o','gpt-4o-mini','gpt-5','gpt-5.1',
      'gpt-5.2','gpt-5.3-codex','gpt-5.4','gpt-5.4-pro',
      'gpt-5.5','gpt-5.6-luna','gpt-5.6-sol','gpt-5.6-terra',
      'gpt-5-mini','gpt-5-nano','o3','o3-mini',
      'o4-mini','bedrock.gpt-5.4','bedrock.gpt-5.5','bedrock.gpt-5.6-luna',
      'bedrock.gpt-5.6-sol','bedrock.gpt-5.6-terra'
    ] },
  ...
  // R/SN is deliberately narrow: only the claude-fable-5 variants are cleared for
  // this tier. Everything else (GPT, Gemini, the wider Claude range) is CCE/SN only.
  rsn: [
    { label: 'Claude (Anthropic)', ids: [
      'azure.claude-fable-5','ocns.claude-fable-5','bedrock.claude-fable-5','vertex_ai.claude-fable-5'
    ] },
  ],
}

const EMBED_TIERS = {
  cce: [ { label: 'Embedding models', ids: [
  ...
  // No embedding models are cleared for R/SN, so document embedding / RAG is
  // unavailable at this tier by design - not an oversight.
  rsn: [],
}
...
function normalizeModelId(id) {
  return (id || '').trim().replace(/^(rsn\.vertex_ai\.|rsn\.|cce\.|ocns\.|azure\.|bedrock\.|vertex_ai\.)/, '')
}
...
function fillModelSelect(sel, groups, current) {
  sel.innerHTML = ''
  let found = false
  // A tier can legitimately have NO approved models (R/SN has no cleared
  // embedding models). Say so, rather than showing a bare "Custom…" box that
  // reads like a loading failure.
  if (!groups || !groups.length) {
    const none = document.createElement('option')
    none.value = ''; none.disabled = true; none.selected = !current
    none.textContent = 'None approved for this classification'
