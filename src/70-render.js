...
// One delegated click listener for message action buttons (Copy / Copy-for-Word /
// Edit / Regenerate), wired once on the persistent #messages container — matches
// the chat-list pattern (data-act, no per-button closures). Idempotent.
function wireMessageActions() {
  const el = document.getElementById('messages')
  if (!el || el.dataset.actsWired) return
  el.dataset.actsWired = '1'
  el.addEventListener('click', e => {
    const btn = e.target.closest('.mact')
    if (!btn || !el.contains(btn)) return
    const act = btn.getAttribute('data-act')
    if (act === 'copy')           copyMsg(btn)
    else if (act === 'copy-html') copyMsgHtml(btn)
    else if (act === 'edit')      editLastUser()
    else if (act === 'regen')     regenerateLast()
  })
}
...
function buildMsgEl(role, text, date, sources, fileNames, errored) {
  ...
  if (!(errored && !isUser)) {
    acts.appendChild(mkEl('button', { class: 'mact', 'data-act': 'copy', title: 'Copy raw markdown to clipboard' }, 'Copy'))
    if (isUser) {
      acts.appendChild(mkEl('button', { class: 'mact edit-act', 'data-act': 'edit', style: 'display:none' }, 'Edit'))
    } else {
      acts.appendChild(mkEl('button', { class: 'mact', 'data-act': 'copy-html', title: 'Paste into Word or Outlook to preserve formatting (tables, bold, headings)' }, 'Copy for Word / Outlook'))
      acts.appendChild(mkEl('button', { class: 'mact regen-act', 'data-act': 'regen', style: 'display:none', title: 'Re-send the last message and get a new response' }, 'Regenerate'))
    }
  }
