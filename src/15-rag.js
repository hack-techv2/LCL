      ...
      try {
        await streamSse(res, payload => {
          ...
          if (evt.type === 'progress') {
            ...
            } else if (evt.total > 1 || evt.batchTotal > 1) {
              // Only announce progress for a genuinely multi-chunk/batch embed that has no
              // progress bar. A single-chunk embed (e.g. the per-query embedding on each RAG
              // search) is trivial and was spamming a pointless "batch 1/1 (1/1 chunks)" toast.
            ...
            if ((onProgress || evt.total > 1 || evt.batchTotal > 1) && typeof setHealth === 'function') setHealth('warn', 'Embedding ' + evt.done + '/' + evt.total)
