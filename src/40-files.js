  ...
  if (typeof mammoth !== 'undefined') {
    try {
      ...
      if (result.messages && result.messages.length) {
        warnings.push('DOCX parsed with ' + result.messages.length + ' Mammoth warning(s).')
        // Surface WHAT the warnings were (previously discarded): browser console in
        // full + first 3 into the server log, so docx extraction issues are debuggable.
