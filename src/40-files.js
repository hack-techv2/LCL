...
function setOcrState(s) {
  ...
  // Refresh the health pill's '+ OCR' segment when the engine flips on/off, but only
  // while idle-connected (pill 'ok') so we never clobber a 'Reading'/'Embedding' status.
  try {
...
async function ensureOcrWorker() {
  ...
  try {
    ...
    // bitmap (it guesses badly on screenshots, which carry no DPI metadata), and
    // keep run-together spacing so table-ish captures stay column-separated.
    try {
      if (typeof w.setParameters === 'function') await w.setParameters({
        user_defined_dpi: String(CFG.OCR_DPI_HINT || 300),
...
// Clamp a scale so w*h stays inside the pixel budget (canvas + memory guard).
function ocrFitScale(w, h, want) {
  const cap = CFG.OCR_MAX_PIXELS || 12e6
  if (w * h * want * want <= cap) return want
  return Math.max(1, Math.sqrt(cap / (w * h)))
}

// Prepare an image for OCR. A raw screen capture is about the worst input you can
// hand Tesseract: ~96 DPI, anti-aliased 10-12px glyphs, often light-on-dark. This
// upscales toward a ~300 DPI equivalent, flattens to greyscale, and inverts
// dark-mode captures so text ends up dark-on-light.
//
// Deliberately NO hard threshold here: Tesseract already binarises internally
// (Otsu), and thresholding anti-aliased small text twice tends to erode thin
// strokes and LOSE characters. Upscaling + orientation of contrast is where the
// real gain is.
async function ocrPrepImage(file) {
  const bmp = await createImageBitmap(file)
  const w0 = bmp.width, h0 = bmp.height
  const target = CFG.OCR_TARGET_LONG_EDGE || 2200
  let scale = target / Math.max(w0, h0)
  scale = Math.max(1, Math.min(CFG.OCR_MAX_UPSCALE || 4, scale))   // never shrink
  scale = ocrFitScale(w0, h0, scale)
  ...
  try {
    ...
    // Mostly-dark image => light text on a dark UI; invert so it reads like print.
  ...
  try {
    if (item.pdfDoc) {
      ...
      for (let i = 0; i < item.emptyPageNums.length; i++) {
        ...
        // Render near 300 DPI (72 DPI base x 4.2), but clamp so an oversized page
        // can't exceed the pixel budget and blow the canvas.
        const _base    = page.getViewport({ scale: 1 })
        const _want    = CFG.OCR_SCALE || 4.2
        const viewport = page.getViewport({ scale: ocrFitScale(_base.width, _base.height, _want) })
      ...
      setOcrProgress(1, 1)
      // Prep first - a raw screenshot handed straight to recognize() reads poorly.
      let input = item.ocrFile
      try {
        const prep = await ocrPrepImage(item.ocrFile)
        input = prep.canvas
...
async function queueFilesForPreview(files, target) {
  ...
  if (target === 'docs') {
    ...
    if (scannedItems.length) {
      ...
      if (choice === 'cancel') { previewQueue = []; previewTarget = null; document.getElementById('file-in').value = ''; healthIdle(); if (typeof renderDocPanel === 'function') renderDocPanel(); return }
...
// Return the health pill to its steady connected state - used when an embed is
// cancelled or finished. Skips if other embeds are still running (that loop owns
// the pill), so we never wrongly flip a live 'Embedding' back to idle.
function healthIdle() {
  try {
...
async function commitDocs(files) {
  ...
  if (creds) {
    ...
    if (over && plans.some(p => p.plan.toEmbed.length) && typeof confirmEmbedBatch === 'function') {
      ...
      if (!selectedIds) {
        ...
        healthIdle(); renderDocPanel(); updateDocsBtn(); await persist(); return
      }
    }
  ...
  await persist(); renderDocPanel(); updateDocsBtn(); healthIdle()
}
