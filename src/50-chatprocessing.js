...
async function summariseInto(sysPrompt, label, text, instruction, bodyEl, signal) {
  ...
  const uiLabel = 'Summarising ' + (typeof esc === 'function' ? esc(label) : label)
...
async function summariseText(sysPrompt, label, text, instruction, bodyEl, signal, depth) {
  ...
  for (let p = 0; p < parts.length; p++) {
    if (signal && signal.aborted) return null
    if (liveEl) liveEl.innerHTML = fmt('_' + label + ' - summarising part ' + (p + 1) + ' of ' + parts.length + '..._')
    const s = await summariseText(sysPrompt, label + ' (part ' + (p + 1) + '/' + parts.length + ')', parts[p], 'Summarise this part of a document.', liveEl, signal, depth + 1)
    if (s == null) return null
    partSummaries.push('Part ' + (p + 1) + ': ' + s)
    if (doneEl) doneEl.innerHTML = fmt(partSummaries.join('\n\n'))
    if (liveEl) liveEl.innerHTML = ''
  }
  if (liveEl) liveEl.innerHTML = fmt('_Combining ' + parts.length + ' part-summaries..._')
  return await summariseText(sysPrompt, label + ' (combined)', partSummaries.join('\n\n'), 'Combine these part-summaries into one cohesive summary of the whole document.', liveEl || bodyEl, signal, depth + 1)
}
...
async function waitForEmbedsIdle(bodyEl, header, signal) {
  ...
  while (embedsActive() && !(signal && signal.aborted)) {
    if (bodyEl) bodyEl.innerHTML = fmt(header + '_Waiting for document embedding to finish before summarising…_')
