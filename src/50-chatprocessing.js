...
async function send() {
  ...
  if (pendingRetry) {
    ...
    if (typeof lclCrumb === 'function') lclCrumb('send_blocked_ratelimit', { secs: _secs })
    toast(_secs > 0
      ? ('Rate-limited \u2014 it\u2019ll retry automatically in ~' + _secs + 's. Press Stop to cancel and send this instead.')
      : ('Rate-limited \u2014 retrying automatically. Press Stop to cancel and send this instead.'), 'info')
    return
  }
