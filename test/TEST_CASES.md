...
### C30 (7 Jul) — developer endpoint UI (URL model): `renderEndpointSection` fills the selector (presets by name + Custom…) and selects the active Model URL; presets render a read-only summary (model/embed URLs, amber "RAG disabled" when no embed URL); Custom reveals Name / Model URL / Embeddings URL / Model fields (`endpointSelChanged`); Save POSTs `{name, modelUrl, embedUrl, model}` to `/api/endpoint`; `endpointBadge()` returns " · <name>" only when off-default. Runs the real `// === endpoint-dev ===` block from 80-ui.js. Server side: **T36** default + URL presets, **T37** set kepler http URL → persists → reset clears, **T38** allowlist refuses non-gov.sg/malformed URLs (suffix spoofs, ftp, credentials, schemeless, bad embed URL) with 400, **T39** an endpoint without an embeddings URL refuses `/api/embed` + `/api/embed-batch` with a clear 400.
