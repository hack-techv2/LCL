...
| U13 | Settings → set a new embed key → Save | A new/changed embed key is verified with one `/api/embed` call: valid → "Embedding key verified"; bad/truncated → "Embedding key rejected (… 401) — check the key is complete" (caught at save, not on first RAG embed) |
